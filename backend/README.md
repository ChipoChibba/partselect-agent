...
The `scraper/` directory contains utility scripts for collecting structured product data from appliance part websites.

This scraper is used to:
- update or expand `product_data.json`
- gather model compatibility lists
- fetch descriptions, manufacturers, and symptom data
- normalize scraped HTML into clean structured JSON
...
```
cd backend/scraper
node scrapePartSelect.js
```

Then the output is stored in ```backend/data/product_data.json```

Note: The scraper module is implemented but intentionally disabled in production out of respect for PartSelect’s Terms of Service. It serves as a technical prototype rather than a data-collection tool.
...
More information on this is given in the root ```README.md``` file. 
...
However, this data was generated by ChatGPT 5.1 to mimic that of the PartSelect. It doesnot consist of a comprehensive list of parts.
